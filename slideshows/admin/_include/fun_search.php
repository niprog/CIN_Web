...
function print_table($list) {			// print table of found items
	...
	for($i=0;$i<$cnt;++$i) {
		...
		if(get_is_dir($dir,$item)) {
			$img = '<div class="sprites dir_png"></div>'; 
			$link = make_link("list",get_rel_item($dir, $item),NULL);
		} else {
			// $img = get_mime_type($dir, $item, "img");
			$img = '<div class="sprites '.get_mime_type($dir, $item, "img").'"></div>';
...
function search_items($dir) {			// search for item
	...
	echo '<div id="ctr" align="center">
	      ...
	      <div class="clr"></div>';
	// Search Box
	echo "<br><table summary=''><form name=\"searchform\" action=\"".make_link("search",$dir,null);
	echo "\" method=\"post\">";
  ...
  echo "\n<tr><td><input name=\"searchitem\" type=\"text\" size=\"25\" value=\"";
	...
	// Results
	if($searchitem!=NULL) {
		echo "<table summary=\"\" width=\"95%\"><tr><td colspan=\"2\"><hr></td></tr>\n";
		if(count($list)>0) {
			// table header
